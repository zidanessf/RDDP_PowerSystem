...
opt =  quote
    function some_model()
        ...
        @variable(lm, y[1:sz])
        @variable(lm, z[1:sz])
        @objective(um, Min, 4sum(x) -3sum(y) - sum(z))

        @objective(lm, Min, sum(y)^2 + sum(z))

        @constraint(lm, sum((2+i/20)*x[i]+y[i]+z[i] for i = 1:sz) == 1)
        @constraint(lm, sum(x[i]+(2+i/30)*y[i] for i = 1:sz) == 1)
        @constraint(lm,cref1[i=1:sz], x[i] >= 0)
        @constraint(lm,cref2[i=1:sz], y[i] >= 0)
        @constraint(lm,cref3[i=1:sz], y[i] == 2*z[i])
...
for i = 1:10
    m = deepcopy(mdl)
    optimize!(m,Gurobi.Optimizer())
end
# for k in 1:5
#     mdl_temp = deepcopy(mdl)
#     @constraint(Lower(mdl_temp),cref[i in 1:10],mdl_temp[:y][i]==1 + k/10)
#     # for i in 1:10
#     #     fix(Lower(mdl_temp)[:y][i],1+k/10)
#     # end
#     optimize!(mdl_temp,Gurobi.Optimizer())
# end
...
#             idx = 2
#             printstyled("****ADDITION OF CONSTRAINT CON****\n";color=:blue)
#             opt[ii] = @add_lower_cons(opt[ii],con,sum(opt[ii][:y][i] + sth[idx]^2 for i in 1:n) == 10 + sin(0.1*(i+j)))
#             # println(opt[ii])
#             f = eval(opt[ii])
#             optimize!(f(),Gurobi.Optimizer(OutputFlag=0))
#         end
